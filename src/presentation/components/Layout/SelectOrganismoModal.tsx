...
export const SelectOrganismoModal: React.FC<SelectOrganismoModalProps> = ({
  ...
  const dependenciasDelOrganismoIds = selectedOrganismo ? getAllDescendantDependencias(selectedOrganismo).map(id => String(id).toLowerCase().trim()) : [];
  ...
  const dependenciasDelOrganismo = dependencias.filter(d => 
    dependenciasDelOrganismoIds.includes(String(d.id).toLowerCase().trim()) &&
